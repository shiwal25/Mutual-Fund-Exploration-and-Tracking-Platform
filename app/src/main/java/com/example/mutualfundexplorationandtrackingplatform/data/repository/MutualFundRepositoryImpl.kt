...
import com.example.mutualfundexplorationandtrackingplatform.data.cache.CachePolicy
...
override suspend fun fetchAndCacheDetails(schemeCode: Int?): Result<MutualFundDetail> =
    withContext(Dispatchers.IO) {
        ...
        if (cached != null && cached.detailsIsFetched && !CachePolicy.isStale(cached.lastSyncEpoch)) {
        ...
        try {
            ...
            mutualFundDAO.updateSyncTime(schemeCode, CachePolicy.now())

            val updated = mutualFundDAO.getFundByScheme(schemeCode)
                ?: error("Data missing after update")
            Result.success(updated)
        } catch (e: Exception) {
...
//}
    ...
    override suspend fun fetchAndCacheCategoryFunds(category: String): Result<List<MutualFundDetail>> =
        withContext(Dispatchers.IO) {
            val cachedFunds = mutualFundDAO.getFundsByCategory(category)

            if (cachedFunds.isNotEmpty()) {
                val oldestSync = mutualFundDAO.getOldestSyncForCategory(category) ?: 0L
                if (!CachePolicy.isStale(oldestSync)) {
                    return@withContext Result.success(cachedFunds)
                }
            }
            ...
            try {
                ...
                val dtos = mutualFundApiService.getFundsByCategory(searchQuery)
                if (dtos.isEmpty()) return@withContext Result.success(emptyList())

                mutualFundDAO.insertFunds(dtos.map { it.toEntityWithCategory(category) })
                mutualFundDAO.updateCategorySyncTime(category, CachePolicy.now())

                Result.success(mutualFundDAO.getFundsByCategory(category))
            } catch (e: Exception) {
                if (cachedFunds.isNotEmpty()) Result.success(cachedFunds) else Result.failure(e)
            }
        }
...
//    }
    ...
    override suspend fun fetchNavData(
        ...
        withContext(Dispatchers.IO) {
            try {
                Result.success(mutualFundApiService.getNavData(schemeCode, startDate, endDate).data)
            } catch (e: Exception) {
                Result.failure(e)
            }
        }
...
//    }
    ...
    override suspend fun searchFunds(query: String): List<MutualFundDTO> =
        withContext(Dispatchers.IO) {
            try {
                val dtos = mutualFundApiService.getFundsByCategory(query)
                mutualFundDAO.insertFunds(dtos.map { it.toEntity() })
                dtos
